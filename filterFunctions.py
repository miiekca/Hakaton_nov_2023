...
def complex_filter(area: str = None, city: str = None, priceFrom: int = None, priceTo: int = None,
    ...
    if condition:
        request = 'SELECT apps.area, apps.priceFrom, apps.priceTo, apps.serviceID, users.city FROM apps, users WHERE ' + " AND ".join(
            condition)
        print(request)
